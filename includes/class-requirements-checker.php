...
if ( ! class_exists( 'Requirements_Checker' ) ) {
	 ...
	 * @since      1.0.0
	 * @package    Order_Form
	 * @subpackage Order_Form/Includes
	 */
	class Requirements_Checker {
		 ...
		 *
		 * @param string $order_form Name of the plugin.
		 * @param string $plugin_slug Slug of the plugin.
		 * @return boolean
		 * @since 1.0.0
		 */
		private function is_plugin_active( $order_form, $plugin_slug ) {
			...
			$this->add_error_notice(
				$order_form . ' is a required plugin.',
				$order_form . ' needs to be installed & activated.'
			);
		 ...
		 *
		 * @param string $order_form Order Form.
		 * @param string $plugin_slug Plugin Slug.
		 * @param string $min_plugin_version Minimum version required of the plugin.
		 * @return boolean
		 * @since 1.0.0
		 */
		private function is_required_plugin_version_active( $order_form, $plugin_slug, $min_plugin_version ) {
			...
			$this->add_error_notice(
				"{$order_form} {$min_plugin_version}+ is required.",
				"{$order_form} {$installed_plugin_version} is installed."
			);
		...
		private function are_required_plugins_dependency_met() {
			...
			$installed_plugins = array_filter(
				...
				function( $required_plugin_data, $required_order_form ) {
					return $this->is_plugin_active( $required_order_form, $required_plugin_data['plugin_slug'] );
				},
			...
			$plugins_installed_with_required_version = array_filter(
				$installed_plugins,
				function( $required_plugin_data, $required_order_form ) {
					return $this->is_required_plugin_version_active( $required_order_form, $required_plugin_data['plugin_slug'], $required_plugin_data['min_plugin_version'] );
				},
				ARRAY_FILTER_USE_BOTH
			);
