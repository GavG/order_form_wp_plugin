...
if ( ! class_exists( 'Dependency_Loader' ) ) {
	...
	class Dependency_Loader {
		...
		public function load_dependencies( $class ) {
			...
			if ( 'Order_Form' !== $parts[0] ) {
				return;
			}

			// Remove 'Order_Form' from parts.
			array_shift( $parts );
			...
			$file_path = Order_Form::get_plugin_path() . implode( '/', $parts ) . $class_file_name;
			...
			$file_path = Order_Form::get_plugin_path() . implode( '/', $parts ) . $trait_file_name;
		...
		protected function load_registries() {
			require_once( Order_Form::get_plugin_path() . 'core/registry/trait-base-registry.php' );
			require_once( Order_Form::get_plugin_path() . 'core/registry/class-controller.php' );
			require_once( Order_Form::get_plugin_path() . 'core/registry/class-model.php' );
		}
		...
		protected function load_core() {
			...
			foreach ( glob( Order_Form::get_plugin_path() . 'core/*.php' ) as $path ) {
				require_once $path;
			}
		}
